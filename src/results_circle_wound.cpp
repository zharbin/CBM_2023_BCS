...
int main(int argc, char *argv[])
{
	...
	//
	double k0 = 0.006375; // 0.0511; neo hookean for skin, used previously, in MPa
	...
	double t_rho = (1.28571E-5/55.05125); // 0.0045 force of fibroblasts in MPa, this is per cell. so, in an average sense this is the production by the natural density
	double t_rho_c = (1.28571E-5*3.28571/55.05125); // 0.045 force of myofibroblasts enhanced by chemical, I'm assuming normalized chemical, otherwise I'd have to add a normalizing constant
	...
	std::vector<double> hexDimensions = {-90.0,90.0,-90.0,90.0,-20.0,20.0};
	...
	double x_center = -17.5;
	double y_center = 22.5;
	double z_center = 35.5;
	double x_axis = 30.21096 + tol_boundary;
	double y_axis = 30.21096 + tol_boundary;
	double z_axis = 30.21096;
	...
	for(int nodei=0;nodei<myMesh.n_nodes;nodei++){
		...
		if(myMesh.boundary_flag[nodei] == 1){ //   myMesh.boundary_flag[nodei]>1 && myMesh.boundary_flag[nodei]<6
...
//			}
            //-----------------//
            // Alternative version with an ellipse instead of a cylinder
            double check_ellipsoid_ip = pow((X_IP(0)-x_center),2)/(x_axis*x_axis) + pow((X_IP(1)-y_center),2)/(y_axis*y_axis) + pow((X_IP(2)-z_center),2)/(z_axis*z_axis);
	...
	myTissue.time_final = 7*24*16; // in hours
